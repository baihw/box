...
public class SimpleStringUtils implements IStringUtils {
    ...
    @Override
    public String joinString(String separator, Object... items) {
        if (null == items || 0 == items.length)
            return "";
        StringBuilder _builder = new StringBuilder(256);
        _builder.append(items[0]);
        for (int _i = 1, _iLen = items.length; _i < _iLen; _i++) {
            _builder.append(separator).append(items[_i]);
        }
